import type { AIConfig, WebAccessMode } from '@shared/types'

export const CHAT_PREFERENCE_KEY = 'stock-monitor:ai-chat-preference:v1'

export type ChatPreference = { model: string; web_access_mode: WebAccessMode; updated_at: string }
...
export function saveChatPreference(model: string, webAccessMode: WebAccessMode): void {
  if (!model) return
  try { localStorage.setItem(CHAT_PREFERENCE_KEY, JSON.stringify({ model, web_access_mode: webAccessMode, updated_at: new Date().toISOString() })) } catch { /* restricted storage */ }
}
