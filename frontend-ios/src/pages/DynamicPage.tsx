...
function PortfolioAnalysisPanel() {
  ...
  const run = () => {
    ...
    if (subsection === 'stress') launch.mutate({ type: subsection, body: { ...base, mode: 'proxy_scenario', scenario_code: 'recession', market_shock: -0.2, nasdaq_shock: -0.25, sector_shocks: {}, style_shocks: {}, interest_rate_change_bp: 100, currency_shocks: {}, volatility_change: 0, use_fundamental_modifiers: true } })
    if (subsection === 'scenario') launch.mutate({ type: subsection, body: { ...base, scenario_code: 'recession', use_fundamental_modifiers: true } })
    if (subsection === 'monte_carlo') launch.mutate({ type: subsection, body: { ...base, horizon_years: 1, simulations: 1000, method: 'block_bootstrap', block_length: 10, rebalance_frequency: 'quarterly', monthly_contribution: 0, target_value: null, confidence_levels: [.8, .95], random_seed: 42, force_refresh: true } })
