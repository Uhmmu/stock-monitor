...
export function ScenarioAnalysisView({ portfolioId }: { portfolioId: number }) {
  ...
  return <div className="scenario-page">
    <div className="scenario-cards">{cards.map(row => {
    ...
    {selected && <div className="scenario-result-toolbar"><div><b>{selectedPreset?.name}</b><span>{savedRun ? cacheTimeLabel(savedRun) : '该情景最近 7 天没有可用结果，可手动开始估算'}</span></div><button onClick={() => launch.mutate(selected)} disabled={isRunning}>{isRunning ? runningCode === selected ? '正在重新估算…' : '另一情景计算中' : savedRun ? '重新估算' : '开始估算'}</button></div>}
