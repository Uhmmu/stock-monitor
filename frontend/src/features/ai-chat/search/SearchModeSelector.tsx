...
export function SearchModeSelector({ mode, config, disabled, onMode }: { mode: WebAccessMode; config: WebSearchConfig; disabled?: boolean; onMode: (mode: WebAccessMode) => void }) {
  ...
  return <div className="ai-search-mode-picker" ref={root}>
    <button type="button" className="ai-search-mode-trigger" disabled={disabled} onClick={() => setOpen(value => !value)} aria-haspopup="listbox" aria-expanded={open} aria-label={`联网模式：${SEARCH_MODE_COPY[mode].label}`} title={`联网模式：${SEARCH_MODE_COPY[mode].label}`}>
      <span className="ai-search-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><circle cx="10.5" cy="10.5" r="5.75"/><path d="m15 15 4.25 4.25"/></svg></span><b>{SEARCH_MODE_COPY[mode].label}</b><i aria-hidden="true">⌃</i>
    </button>
