...
function PulseCard({ row, onSelect, compact = false }: { row: PulseRow; onSelect: (row: PulseRow) => void; compact?: boolean }) {
  return <button type="button" className={`industry-pulse-card${compact ? ' compact' : ''}`} onClick={() => onSelect(row)}>
    ...
    <div className="industry-score-grid"><ScoreCell label="Strength" value={row.strength ?? row.pulse}/><ScoreCell label="Heat" value={row.heat}/><ScoreCell label="Risk" value={row.risk} inverse/></div>
...
type DetailData = { node: PulseRow; history: { date: string; value: number }[]; etfs: JsonRecord[]; summary: string | null; relative: JsonRecord }
...
function detailData(value: unknown, fallback: PulseRow): DetailData {
  ...
  return { node: node.id === 'node-0' && fallback.id !== 'node-0' ? fallback : node, history, etfs, summary: asText(first(record.ai_summary, record.summary, record.narrative)), relative }
}
...
function NodeDetail({ selected, range, onRange, onClose }: { selected: PulseRow | null; range: PulseRange; onRange: (range: PulseRange) => void; onClose: () => void }) {
  ...
  const data = selected && detail.data ? detailData(detail.data, selected) : selected ? { node: selected, history: [], etfs: [], summary: null, relative: {} } : null
  ...
  return <Sheet open={selected !== null} onClose={onClose} title={selected?.name || '行业板块详情'} size="wide">
    {!selected ? null : detail.isLoading ? <LoadingState text="正在读取板块详情…"/> : detail.isError ? <ErrorState text="板块详情暂时无法读取。"/> : data && <article className="industry-node-detail">
      ...
      <div className="industry-detail-facts"><div><span>状态</span><b>{moodText(data.node.mood)}</b></div><div><span>5D 变化</span><b>{signedText(data.node.change5d)}</b></div><div><span>Breadth</span><b>{scoreText(data.node.breadth)}</b></div><div><span>Relative Strength</span><b>{scoreText(data.node.relativeStrength)}</b></div></div>
