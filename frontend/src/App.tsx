...
function StockManagement({onWatchlistChanged}:{onWatchlistChanged:()=>void}) {
  ...
  const StockNote = ({stock,index,matched=false}:{stock:ManagedStock;index:number;matched?:boolean}) => <button key={stock.ticker} className={`managed-stock-note note-tone-${index%4}${matched?' matched':''}`} onClick={()=>setSelected(stock)}><div className="stock-note-head"><span>{stock.ticker}</span><i aria-hidden="true">•••</i></div><small>{stock.company_name||'公司资料待同步'}</small><strong>{formatPrice(stock.price)}</strong><em className={stock.change_percent==null?'':stock.change_percent<0?'negative':'positive'}>{stock.change_percent==null?'今日行情待同步':`${stock.change_percent>=0?'+':''}${stock.change_percent.toFixed(2)}% 今日`}</em><span className="stock-note-footer"><span>{matched?`比较：${stock.peer_referenced_by.join('、')||'—'}`:stock.alert_enabled?'波动提醒已开启':'波动提醒关闭'}</span><b>{matched?'匹配股票':'设置 →'}</b></span></button>
