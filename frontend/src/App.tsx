...
type Dashboard = { market:{is_open:boolean; checked_at:string}; stocks:{ticker:string;company_name?:string|null;logo_url?:string|null;price:number|null;previous_close:number|null;price_source?:string|null;previous_close_source?:string|null;updated_at:string|null;volume:number|null;volume_ratio:number|null;volume_label:string|null}[] }
...
const marketSourceLabel = (value:string|null|undefined) => {
  const [provider,kind] = (value||'').split(':')
  const name:Record<string,string> = {alpaca:'Alpaca',finnhub:'Finnhub',yfinance:'Yahoo',fmp:'FMP'}
  return `${name[provider]||provider||'未知'}${kind==='historical'?' 日线':kind==='snapshot'?' 快照':kind?` ${kind.toUpperCase()}`:''}`
}
...
function NavIcon({name}:{name:string}) {
  const paths:Record<string,React.ReactNode> = {
    ...
    industry:<><path d="M4 19V5M4 19h16"/><path d="m7 15 4-4 3 2 5-6"/></>,
    mood:<><path d="M3 17c3-7 5 2 8-5s5 2 10-5"/><path d="M3 21h18M6 7V3M12 8V4M18 6V2"/></>,
    'mood-lab':<><path d="M4 20h16M7 20V9l5-5 5 5v11"/><path d="M9 13h6M9 16h6"/><circle cx="12" cy="8" r="1"/></>,
...
export default function App({ appleDesign = false, betaDesign = false }: { appleDesign?: boolean; betaDesign?: boolean }) {
  ...
  const navFlyoutExitTimer = useRef<ReturnType<typeof setTimeout>|null>(null)
  ...
  const navFlyouts = [navFlyoutExit,navFlyout].filter((value):value is NonNullable<typeof navFlyout>=>value!==null)
  ...
  return <div className={`app${appleDesign?' apple-design-app':''}${betaDesign?' beta-design-app':''}${navCollapsed?' apple-nav-collapsed':''}`}>
    ...
    {betaDesign?<BetaCommandBar groups={betaNavGroups} tab={tab} title={tabTitle} market={viewDashboard?.market} username={authUser.username} demoMode={demoMode} mobileOpen={mobileNavOpen} onSelect={selectTab} onToggleMenu={()=>setMobileNavOpen(value=>!value)} onLogout={logout}/>:<aside className={mobileNavOpen?'mobile-open':''}>
      <div className="brand"><div className="brand-orb"><img src="/logo.png" className="brand-logo" alt="logo"/></div><div className="brand-copy"><div className="brand-title-row"><strong>小日向美香</strong><button className={`crypto-hub-btn${tab==='crypto'?' active':''}`} onClick={()=>selectTab('crypto')} aria-label="进入 Crypto 研究面板" aria-pressed={tab==='crypto'} title="Crypto 研究">₿</button></div><small>{betaDesign?'BETA CONTROL ROOM':'Powered by 和泉妃爱'}</small></div>{appleDesign&&<button className="nav-collapse-btn" onClick={()=>setNavCollapsed(value=>{localStorage.setItem(navStorageKey,value?'0':'1');setNavFlyout(null);setNavFlyoutExit(null);return !value})} aria-label={navCollapsed?'展开侧栏':'收起侧栏'} aria-pressed={navCollapsed}><span aria-hidden="true">‹</span></button>}<div className="mobile-quick-stats"><span><b>{viewDashboard?.stocks.length||0}</b><small>监控</small></span><span><b>{viewAlerts?.length||0}</b><small>异动</small></span><span><b>{groupInvestigations(investigations.data).length}</b><small>调查</small></span></div><button className="mobile-menu-btn" onClick={()=>setMobileNavOpen(v=>!v)} aria-expanded={mobileNavOpen}>{mobileNavOpen?'关闭':'菜单'}</button></div>
    ...
    <main className={tab==='ai'?'ai-main':''}>
      ...
      {tab==='overview'&&<>
        ...
        {!demoMode&&<PortfolioBenchmarkOverview/>}
        <section><div className="section-title"><div><p>WATCHLIST</p><h2>市场快照</h2></div><div className="section-actions"><button className="stock-collapse-btn" onClick={()=>setStocksExpanded(v=>!v)}>{stocksExpanded?'收起':'展开更多'} <span>{stocksExpanded?'↑':'↓'}</span></button><button onClick={()=>selectTab('watchlist')}>管理自选股 <span>→</span></button></div></div><div className={`stock-grid${stocksExpanded?' is-expanded':' is-collapsed'}`}>{overviewStocks.map(stock=>{const change=stock.price&&stock.previous_close?(stock.price-stock.previous_close)/stock.previous_close*100:null;return <article className="stock-card" key={stock.ticker}><div className="stock-card-head"><ProfileLogo symbol={stock.ticker} url={stock.logo_url}/><div className="stock-card-id"><span className="ticker">{stock.ticker}</span><small>{stock.updated_at?'刚刚更新':'等待首次采集'}</small></div></div><strong>{formatPrice(stock.price)}</strong><span className={change!=null&&change<0?'negative':'positive'}>{change==null?'—':`${change>=0?'+':''}${change.toFixed(2)}% 今日`}</span><small className="stock-source">现价 {marketSourceLabel(stock.price_source)} · 昨收 {marketSourceLabel(stock.previous_close_source)}</small><button className="company-overview-btn" onClick={()=>setSelectedProfileSymbol(stock.ticker)}>公司概览</button>{stock.volume_label&&stock.volume_label!=='正常'&&<span className={`vol-tag ${stock.volume_label==='放量'?'heavy':'light'}`}>{stock.volume_label} · {stock.volume_ratio?.toFixed(2)}×</span>}<div className="card-glow"/></article>})}{!overviewStocks.length&&<div className="empty">添加第一只股票，开始建立你的市场雷达。</div>}</div></section>
      ...
      {tab==='industry'&&<IndustryPulse enabled={!demoMode}/>}
      ...
      {tab==='settings'&&<><AppearanceSettings/>{settings.data&&<SettingsForm initial={settings.data} onSaved={()=>client.invalidateQueries({queryKey:['settings']})}/>}<AdanosQuotaPanel/><CongressSettings/><RealtimeProviderHealthPanel/><MacroDataSourcePanel isAdmin={authUser.role==='admin'}/><DiscoverySettingsPanel/>{authUser.role==='admin'&&<AdminOperationsPanel/>}</> }
...
function AppearanceSettings() {
  return <section className="settings-card appearance-settings"><div><h2>外观</h2><p>切换浅色或深色界面。</p></div><ThemeToggle className="settings-theme-toggle"/></section>
}
