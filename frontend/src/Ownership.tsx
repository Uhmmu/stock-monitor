...
export function OwnershipSection({symbol}:{symbol:string}) {
  ...
  return <section className="ownership-section">
    ...
    {view==='summary'&&<>
      ...
      {!!cards.length&&<div className="ownership-card-grid">{cards.map(([label,value])=><article key={label} title={descriptions[label]}><span>{label}<i aria-label="指标说明">?</i></span><strong>{value}</strong><small>来源：Yahoo · {data?.as_of_date?`截至 ${data.as_of_date}`:'数据日期未披露'}</small></article>)}</div>}
    ...
    {view==='insider'&&<OwnershipTable loading={insider.isLoading} empty={!insider.data?.items.length} message={insider.data?.message||'暂无 SEC Form 4 内部人交易。'} notice={insider.data?.delayed_notice} columns={['内部人','交易类型','日期','股数','价格','交易金额','来源']} rows={(insider.data?.items||[]).map(row=>[`${row.insider_name}${row.insider_title?` · ${row.insider_title}`:''}`,row.transaction_type,row.transaction_date||'—',compact(row.shares),compact(row.price),compact(row.transaction_value),'SEC EDGAR'])}/>}
    {view==='13f'&&<OwnershipTable loading={holdings.isLoading} empty={!holdings.data?.items.length} message={holdings.data?.message||'暂无匹配的 SEC 13F 机构申报。'} notice={holdings.data?.delayed_notice} columns={['申报机构','报告期','申报日','股数','申报市值','Put/Call','来源']} rows={(holdings.data?.items||[]).map(row=>[row.filer,row.report_period,row.filing_date||'—',compact(row.shares),compact(row.reported_value),row.put_call||'—','SEC EDGAR'])}/>}
  </section>
}
...
function OwnershipTable({loading,empty,message,notice,columns,rows}:{loading:boolean;empty:boolean;message:string;notice?:string;columns:string[];rows:string[][]}) {
  ...
  const columnsStyle={gridTemplateColumns:`minmax(180px,1.4fr) repeat(${columns.length-1},minmax(94px,1fr))`}
  return <div className="ownership-table-wrap">{notice&&<p className="ownership-delay-note">{notice}</p>}{empty?<div className="empty">{message}</div>:<div className="ownership-table"><div className="ownership-table-head" style={columnsStyle}>{columns.map(column=><span key={column}>{column}</span>)}</div>{rows.map((row,index)=><div className="ownership-table-row" style={columnsStyle} key={index}>{row.map((value,cell)=><span key={cell}>{value}</span>)}</div>)}</div>}</div>
}
